...
const provinceCoordinates = {
    ...
    "江苏省": [32.0617, 118.7636]
    // ... ✅ 你可以继续添加完整省份
};

async function loadChinaMapAndStats() {
    const res = await fetch("/api/china/province/stats");
    const provinces = await res.json();

    const max = Math.max(...provinces.map(p => p.confirmed || 0));

    provinces.forEach(p => {
        const latlng = provinceCoordinates[p.province];
        if (!latlng) return;

        const radius = Math.max((p.confirmed / max) * 300000, 30000);
